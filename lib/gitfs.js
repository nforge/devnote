...
var memo = {};
var memoIdQueue = [];
var MEMOIZE_NUMBERS_LIMIT = 100000;
var MEMOIZE_ENTITY_MAXSIZE = 40960;
var MEMOIZE = { NEVER: 0, AUTO: 1, ALWAYS: 2 };
var USE_MEMOIZATION = MEMOIZE.ALWAYS;
...
var init = function(repoPath, callback) {
  REPO_PATH = repoPath;
  fs.exists(REPO_PATH, function(exists) {
...
var getCommitIdFromHEAD = function(callback) {
  ...
  if (fs.existsSync(REPO_PATH + '/HEAD')) {
    ...
    if (fs.existsSync(path.join(REPO_PATH, 'packed-refs'))) {
...
var findBlobInUnmemoizedTreeById = function(treeId, targetId, callback) {
  ...
  if (fs.existsSync(_getObjectPath(treeId))) {
...
var _readObject = function(id, callback) {
  ...
  var cb = function(err, type, body) {
    ...
    if (USE_MEMOIZATION) {
      if (body.length <= MEMOIZE_ENTITY_MAXSIZE) {
        ...
        console.warn('MEMOIZE_ENTITY_MAXSIZE exceeded:', MEMOIZE_NUMBERS_LIMIT, body.length);
      }
    }
  ...
  if (fs.existsSync(_getObjectPath(id))) {
...
var findBlobInTreeById = function(treeId, blobId, callback) {
  ...
  if (memo[treeId]) {
    findBlobInMemoizedTreeById(memo[treeId], blobId, callback);
  } else {
    if (USE_MEMOIZATION == MEMOIZE.ALWAYS) {
      readObject(treeId, function(err, tree) {
        findBlobInMemoizedTreeById(tree, blobId, callback);
      });
    } else {
...
var findBlobInTreeByName = function(treeId, blobName, callback) {
  ...
  var tree = memo[treeId];

  if (tree) {
    findBlobInMemoizedTreeByName(tree, blobName, callback);
  } else {
    if (USE_MEMOIZATION == MEMOIZE.ALWAYS) {
      readObject(treeId, function(err, tree) {
        findBlobInMemoizedTreeByName(tree, blobName, callback);
  ...
  readObject(query.until, function(err, commit) {
    ...
    findBlobByNameOrId(commit, null, query.filename, function(err, id, name) {
      ...
      new Graph(head).on('visit', function(prev, curr) {
        ...
        var limitExceeded = (query.limit !== null && query.limit <= 0);
        var inRange = query.since !== curr.commitId;
        if (curr.id && !limitExceeded && inRange) {
          query.limit -= 1;
          history.push(curr);
        }
