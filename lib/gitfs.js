...
var _findInTree = function(buffer, blobName) {
    var begin = null;
    var expected = new Buffer(blobName, 'utf8');
    var matching = false;
    var result = null;

    for (var pointer = 0; pointer < buffer.length; pointer++) {
        if (buffer[pointer] == 0) {
            if (matching) {
...
var _parseTreeBody = function(buffer) {
    ...
    for (var pointer = 0; pointer < buffer.length; pointer++) {
        if (begin == null && buffer[pointer] == 32) {
            begin = pointer + 1;
        } else if (begin != null && buffer[pointer] == 0) {
            filename = buffer.toString('utf8', begin, pointer);
            id = buffer.toString('hex', pointer + 1, pointer + 1 + SHA1CHARACTER_LENGTH);
            tree[filename] = id;
            pointer += SHA1CHARACTER_LENGTH;
            begin = null;
        }
    }
...
var _readObjectFromPack = function(id, callback) {
    ...
    var _find = function(files) {
        if (!files || files.length == 0) {
            return callback(new Error("pack: Object '" + id + "' does not exist."));
        }
...
var findInTree = function(id, blobName, callback) {
    ...
    var cb = function(err, type, body) {
        if (err) return callback(err);
        callback(null, _findInTree(body, blobName));
    }
    ...
    var cb = function(err, type, body) {
        ...
        if (body.length <= MEMOIZE_ENTITY_MAXSIZE) {
            while (memoIdQueue.length >= MEMOIZE_NUMBERS_LIMIT) {
                console.warn('MEMOIZE_NUMBERS_LIMIT exceeded:',
                        MEMOIZE_NUMBERS_LIMIT);
...
var _queryLog = function(query, callback) {
    ...
    readObject(query.until, function(err, commit) {
        ...
        __getBlobId(commit.tree, function(err, blobId) {
            if (err) return callback(err);
            ...
            readObject(commit.parent, function(err, parentCommit) {
                if (err) return callback(err);

                __getBlobId(parentCommit.tree, function(err, nextBlobId) {
...
var log = function(filename, limit, callback) {
    getCommitIdFromHEAD(function(err, id) {
        _queryLog({filename: filename, until: id, limit: limit}, callback);
    });
}
...
var getHeadTree = function(callback){
    getCommitIdFromHEAD(function (err, id) {
        readObject(id, function (err, commit) {
            readObject(commit.tree, callback);
        });
    });
}
