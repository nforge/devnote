...
var init = function (repopath, callback) {
    ...
    fs.mkdir(REPO_PATH, function (err) {
        if (err) {
            if (err.code === 'EEXIST') {
                return callback(new Error(REPO_PATH + " already exists"));
            }
            ...
            return async.series([
                async.apply(async.map,
                    [REPO_PATH + '/objects', REPO_PATH + '/refs'], fs.mkdir),
                async.apply(fs.mkdir, REPO_PATH + '/refs/heads'),
                async.apply(fs.writeFile, REPO_PATH + '/HEAD',
...
var getCommitIdFromHEAD = function (callback) {
    ...
    if (path.existsSync(REPO_PATH + '/HEAD')) {
        ...
        if (path.existsSync(ref)) {
            ...
            if (id) {
                return callback(null, id.slice(0, 40).toString());
            }
        ...
        return callback(new Error(ref + ' does not exist'));
    }

    return callback(new Error('HEAD does not exist'));
};
...
var _serializeCommit = function (commit) {
    ...
    raw += commit.message;
    return new Buffer('commit ' + Buffer.byteLength(raw, ENCODING) + NUL + raw,
            ENCODING);
};
...
var _prepareCommitData = function (request) {
    ...
    writer = request.user.name + ' <' + request.user.email + '> ' +
        unixtime + ' ' + request.user.timezone;
...
var _findInTree = function (buffer, blobName) {
    ...
    begin = null;
    expected = new Buffer(blobName, 'utf8');
    matching = false;
    result = null;
...
var _parseTreeBody = function (buffer) {
    ...
    for (pos = 0; pos < buffer.length; pos++) {
        if (begin === null && buffer[pos] === 32) {
            ...
            filename = buffer.toString('utf8', begin, pos);
...
var _readObjectFromPack = function (id, callback) {
    ...
    var _find = function (files) {
        ...
        if (!files || files.length === 0) {
            return callback(
                    new Error("pack: Object '" + id + "' does not exist.")
            );
        }

        matched = files.pop().match(/pack-([0-9abcdef]{40}).idx/);
    ...
    readObject(commitId, function (err, commit) {
        ...
        readObject(commit.tree, function (err, tree) {
            if (tree[filename]) {
                ...
                callback(new Error(
                    "'" + filename + "' not found in the commit " + commitId
                ));
            }
        });
    });
