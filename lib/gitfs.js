...
var _findInTreeByName = function(tree, blobName) {
  ...
  for (; pointer < tree.length; pointer++) {
    if (tree[pointer] === 0) {
      pointer += 1; // <NUL>
      if (tree.toString(ENCODING, begin, pointer - 1) == blobName) {
...
var _queryLog = function(query, callback) {
  ...
  return readObject(query.until, function(err, commit) {
    ...
    var __findBlobInTree = function(treeId, cb) {
      if (memo[treeId]) {
        ...
        readObject(treeId, function(err, tree) {
          ...
          } else if (query.lastEntry && query.lastEntry.blobId) {
            for (var fn in tree) {
              if (tree[fn] == query.lastEntry.blobId) {
        ...
        findInTreeByName(treeId, query.filename, function(err, matchedId) {
          if (matchedId) {
            ...
            return findInTreeById(treeId, query.lastEntry.blobId, function(_err, matchedName) {
              if (matchedName) {
    ...
    var __addHistory = function(commits, commit) {
      commits.push(commit);
      commits.ids.push(commit.id);
    }
    ...
    return __findBlobInTree(commit.tree, function(err, matchedId, matchedName) {
      ...
      var thisEntry, isNameChanged, isContentsChanged;
      var commits = [];
      commits.ids = [];

      thisEntry = _.clone(commit);
      thisEntry.id = query.until;

      if (matchedName && !matchedId) {
        // case A: a blob which has same contents but different name found.
        thisEntry.blobId = query.lastEntry.blobId;
        thisEntry.filename = matchedName;
      } else if (!matchedName && matchedId) {
        // case B: a blob which has same name found.
        thisEntry.blobId = matchedId;
        thisEntry.filename = query.filename;
      } else if (!matchedName && !matchedId) {
        // case C: no blob matched by name or contents.
        if (query.lastEntry.blobId || query.lastEntry.filename) {
          __addHistory(commits, query.lastEntry);
        }
        query.lastEntry = thisEntry;
        return goNext(commits, query.limit - 1, callback);
      } else {
        // the others: error
        return callback(new Error("gitfs.__queryLog: Unexpected exception"));
      }

      // If lastEntry is stored and something is difference between
      // lastEntry and thisEntry, add thisEntry into history.
      if (query.lastEntry) {
        isNameChanged = query.lastEntry.filename != thisEntry.filename;
        isContentsChanged = query.lastEntry.blobId != thisEntry.blobId;
        if (isContentsChanged || isNameChanged) {
          query.filename = thisEntry.filename;
          __addHistory(commits, query.lastEntry);
          query.limit -= 1;
        }
      }

      // This commit is the final destination of this travel.
      if (!thisEntry.parent) {
        // This commit is the root commit.
        __addHistory(commits, thisEntry);
        query.limit -= 1;
      } else if (thisEntry.parent == query.since) {
        // This commit is the final destination defined by query.since.
        return readObject(thisEntry.parent, function(err, commit) {
          return __findBlobInTree(commit.tree, function(err, matchedId, matchedName) {
            var isNameChanged = matchedName && (thisEntry.filename != matchedName);
            var isContentsChanged = matchedId && (thisEntry.blobId != matchedId);
            var isCreated = !matchedId || !matchedName;

            if (isNameChanged || isContentsChanged || isCreated) {
              __addHistory(commits, thisEntry);
              query.limit -= 1;
            }
