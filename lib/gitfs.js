...
var _serializeTree = function (tree) {
    ...
    _.each(tree, function(blobId, blobName) {
        ...
        ws.write(new Buffer(blobId, 'hex'));
    });
...
var commit = function(request, callback) {
        ...
        commit: function(cb) {
            getCommitIdFromHEAD(function(err, parentId) {
                if (parentId) {
                    ...
                    _getParentTree(parentId, tree, function(err, parentTree) {
                        createCommitFromTree(commitData, parentTree, cb);
                    });
                } else {
    ...
    }, function(err, result) { callback(err, result.commit); } ); // createCommitFromTree의 cb결과
}
...
var _getParentTree = function(parentId, tree, callback) {
    readObject(parentId, function(err, parentCommit) {
        readObject(parentCommit.tree, function(err, parentTree) {
...
var diff = function(name, a, b, type_, callback) {
        ...
        default:
            show(name, a, function(err, contentA) {
                show(name, b, function(err, contentB) {
                    callback(err, jsdiff.diffLines(contentA.toString(), contentB.toString()));
                });
            });
    }
}
