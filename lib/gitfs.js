...
var _parseTreeBody = function(buffer) {
    ...
    for (var POS = 0; POS < buffer.length; POS++) {
        if (buffer[POS] == 0) {
            var NUL_POSITION = POS,
                SHA1CHARACTER_LENGTH = 20;
            var partial = buffer.toString('utf8', 0, NUL_POSITION).split(NUL, 1)[0];
            var filename = partial.substring(partial.indexOf(' ') + 1);
            var id = buffer.slice(NUL_POSITION + 1, NUL_POSITION + 1 + SHA1CHARACTER_LENGTH).toString('hex');
            tree[filename] = id;
            buffer = buffer.slice(NUL_POSITION + 1 + SHA1CHARACTER_LENGTH);
            POS = 0;
        }
    }
    return tree;
