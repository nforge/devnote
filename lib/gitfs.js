...
var _createObjectBucket = function(digest, callback) { 
    var bucketPath = REPO_PATH + '/objects/' + digest.substr(0,2);
...
var _storeObject = function(serialized, callback) {
    var digest = this._sha1sum(serialized);
    var self = this;
...
var _storeFiles = function(files, callback){
    ...
    async.forEach(_.keys(files), function (filename, cb) {
        gitfs._storeObject(gitfs._serializeBlob(files[filename]), function (err, sha1sum) {
...
var _createCommitFromTree = function(commitData, tree, callback) {
    ...
    gitfs._storeObject(gitfs._serializeTree(tree), function(err, sha1sum) {
        commitData.tree = sha1sum;
        gitfs._storeObject(gitfs._serializeCommit(commitData), function(err, sha1sum) {
            fs.writeFile(REPO_PATH + '/refs/heads/master', sha1sum, function(err) {
