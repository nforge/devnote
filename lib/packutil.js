...
var _loadIndex = function (callback) {
    var idxFilename = this.idxFilename;
...
var _getObjectRefDelta = function (pos, callback) {
    var base_id, delta, self, input;

    pos += 1;
    base_id = this.pack.toString('hex', pos, pos + 20);
    pos += 20;

    delta = new WritableStream();
    self = this;
    delta.end = function () {
        var delta = this;
    ...
    input = fs.createReadStream(this.packFilename, {start: pos});
    input.pipe(zlib.createInflate()).pipe(delta);
};

var _getObjectOfsDelta = function (pos, callback) {
    var original_pos, c, base_offset, delta, self, input;
    ...
    // read the delta and apply it to the base object.
    delta = new WritableStream();
    self = this;
    ...
    delta.end = function () {
        ...
        self._getObject(base_pos, function (err, type, base) {
            var patched = deltautil.patch(base, delta.getBuffer());
            return callback(err, type, patched);
        });
    };
...
var _getObjectNotDelta = function (firstByte, pos, callback) {
    ...
    zlib.inflate(this.pack.slice(pos), function (err, result) {
        if (err) {
            return callback(err);
        }
...
var _getObject = function (pos, callback) {
    var c, type;
    ...
    switch (type) {
    case OBJ_OFS_DELTA:
        return this._getObjectOfsDelta(pos, function (err, type, object) {
            return callback(err, type, object);
        });
    case OBJ_REF_DELTA:
        return this._getObjectRefDelta(pos, function (err, type, object) {
            return callback(err, type, object);
        });
    default:
        return this._getObjectNotDelta(c, pos, function (err, object) {
            return callback(err, type, object);
        });
    }
};
...
var getObject = function (id, callback) {
    ...
    if (this.offsets[id] === 0) {
        return callback(new Error("Large file >4GiB is not supported yet."));
    }

    return this._getObject(this.offsets[id], callback);
};
...
var Pack = function () {
    return {
        ...
        _getObjectNotDelta: _getObjectNotDelta,
...
var load = function (repoPath, id, callback) {
    ...
    loader.once("load", function (pack) {
