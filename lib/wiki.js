...
function getPage(name, commitId_, callback) {
    ...
    var show = function(err, commitId) {
        ...
        gitfs._getCommitIdFromHEAD(function(err, head) {
            if (err) return callback(err);
            gitfs._getBlobId(name, head, function(err, blobInHead) {
                gitfs._getBlobId(name, commitId, function(err, blobInCommit) {
    ...
    if (commitId) {
        ...
        gitfs._getCommitIdFromHEAD(show);
    }
}
...
var diff = function(name, a, b, callback) {
    ...
    wiki.getPage(name, a, function(err, pageA) {
        wiki.getPage(name, b, function(err, pageB) {
            callback(err, jsdiff.diffLines(pageA.content, pageB.content));
        });
    });
}
