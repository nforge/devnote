  ...
  return gitfs.getHeadCommit(function(err, commit) {
    ...
    return gitfs.readObject(treeId, function(err, tree) {
      ...
      async.forEach(Object.keys(tree), function(name, cb) {
        self.getHistory(name, 1, function(err, commits) {
          pages.push({
            name: name,
            date: new Date(commits[0].author.unixtime * 1000)
          });
          return cb(err);
        });
      }, function(err) {
        // sort pages by date
        _.sortBy(pages, function(page) {
          return page.date;
        });
